...
The **Tools** grid is now sectioned for scanability, and the **Gear** tab has a first
Room-backed inventory slice. **14 tools** live across the **Tools** grid + **Planner** hub.
...
1. **Gear is only a foundation slice** — basic inventory CRUD exists, but catalog seeding,
   maintenance logs, photos and packing kits are still missing.
...
4. **NEXT → Finish Phase 4 Gear**: catalog seeding/shared models, packing kits with weight,
   and maintenance logs to turn the Gear tab into the real \"track everything\" backbone.
