...
- **[G1] Gear inventory. ✅ richer metadata landed** Bodies, lenses, accessories: model, serial,
...
- **[G3] Battery & memory-card tracker. ✅** Room-backed `GearBatteryEntity` +
...
- **[U4] Live histogram & zebra. ✅** From the CameraX preview already in the app.
  *Update:* the Tools grid now links to a dedicated full-screen route that reuses the shared
  back-camera preview path, binds a throttled `ImageAnalysis` luminance stream in the same camera
  lifecycle, shows a live 32-bin histogram, and overlays a coarse zebra grid for clipped
  highlights while keeping the analysis math in pure `utilities/domain/`.
