...
**Tools** (sectioned home grid + search + favorites + recents), **Planner** (hub), **Gear** (inventory + filters + power/media + loans + insurance/export + kits + maintenance), **Film** (hub + stock library + roll logger + development timer + push/pull helper + reciprocity assistant), **More** (settings/theme + lightweight help).
...
repeat access. A further follow-up now records recently opened tools in ordered DataStore history
and surfaces a **Recent** section that stays filtered by the current search while hiding favorited
routes there to avoid duplicate cards stacked back-to-back.
...
favorite tool routes, and recent tool history.
...
```
**Current status: `assembleDebug` succeeds; 192 unit tests, 0 failures.** CI at
